...
#' @param print_outliers Optional argument to list individual outliers. Default set as TRUE.
#' @param drop_outliers Optional argument to drop outliers and return a data frame minus outliers. Default set as FALSE.
#' @param ... Currently not used.
#'
#' @return Check (message) on whether outliers were detected or not, as well as an optional 
#' data frame minus outliers (i.e., `drop_outliers = TRUE`)
#'
...
#' deletion. If users opt to drop observations that may be problematic, they may do so by specifying `drop_outliers = TRUE`.
#' 
