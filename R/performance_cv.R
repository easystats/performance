...
#'   leave-one-out (`"loo"`). If `data` is suppled
...
#'   if `method = "holdout"` and the `Method` used for cross-validation.
#'
#' @examples
...
performance_cv <- function(
  ...
  if (info$is_linear) {
    if (!is.null(data)) {
      ...
      test_pred <- unlist(mapply(function(.x, .y) {
        insight::get_predicted(.y, data = model_data[.x$test, ])
      }, cv_folds, models_upd, SIMPLIFY = FALSE))
      test_resp <- unlist(lapply(cv_folds, function(.x) {
        as.data.frame(model_data[.x$test, ])[[resp.name]]
      }))
      test_resd <- unlist(test_resp) - unlist(test_pred)
    }
  } else {
  ...
  RMSE <- sqrt(MSE)
  R2 <- 1 - MSE / mean((test_resp - mean(test_resp, na.rm = TRUE))^2)
  out <- data.frame(MSE = MSE, RMSE = RMSE, R2 = R2, method = method)
  out$k <- if (method == "k_fold") k else NULL
  out <- out[, colnames(out) %in% c("k", metrics)]
