...
For frequentist mixed models, `r2()` (resp. `r2_nakagawa()`) computes the _mean_ random effect variances, thus the r-squared value is also appropriate for mixed models with more complex random effects structures, like random slopes or nested random effects (see _Johnson 2014_ and _Nakagawa et al. 2017_).
...
Similar to r-squared, the ICC provides information on the explained variance and can be interpreted as "the proportion of the variance explained by the grouping structure in the population" (_Hox 2002: 15_).
...
Overdispersion can be fixed by either modelling the dispersion parameter (not possible with all packages), or by choosing a different distributional family (like Quasi-Poisson, or negative binomial...).
...
`check_singularity()` checks mixed models (of class `lme`, `merMod`, `glmmTMB` or `MixMod`) for singularity.
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
library(rstanarm)
m3 <- stan_glm(mpg ~ wt + cyl, data = mtcars)
...
```
```{r, warning=FALSE, message=FALSE, echo=FALSE}
knitr::kable(compare_performance(m1, m2, m3), digits = 2)
```
