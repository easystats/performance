...
```{r scatter, fig.cap = "Scatter plot of height and weight, with two extreme observations: one model-consistent (top-right) and the other, model-inconsistent (i.e., and outlier; bottom-right)."}
...
```{r model2, fig.cap = "The leverage method (Cook's distance) correctly distinguishes the true outlier from the model-consistent extreme observation)."}
outliers <- check_outliers(model, method = "cook")
which(outliers)
plot(outliers)
```
