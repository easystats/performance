if (require("testthat") && require("performance") && require("ISLR") && require("bayestestR")) {
  ...
  test_that("roc", {
    expect_equal(head(roc1$Sensitivity), c(0, 0.07692, 0.15385, 0.23077, 0.30769, 0.38462), tolerance = 1e-3)
    expect_equal(head(roc2$Sensitivity), c(0, 0, 0, 0, 0.00154, 0.00154), tolerance = 1e-3)
  })

  test_that("auc", {
    expect_equal(auc1, 0.964, tolerance = 1e-3)
    expect_equal(auc2, 0.535, tolerance = 1e-3)
  })
}
