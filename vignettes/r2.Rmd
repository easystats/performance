...
typically corresponds the proportion of the variance in the dependent variable (the response) that
is *explained* (i.e., predicted) by the independent variables (the
predictors).
...
In the context of a generalized linear model (e.g., a logistic model which outcome is binary), $R^2$ doesn't measure the
...
The marginal $R^2$ considers only the variance of the **fixed effects** (without the random effects), while
...
Note that `r2` functions only return the $R^2$ values. We would encourage users to instead always use the `compare_performance` function to get a more comprehensive set of indices of model fit. 
